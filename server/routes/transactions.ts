...
router.post('/transactions', async (req, res) => {
  const { id, date, type, categoryId, accountId, transferToId, amount, note } = req.body;
...
router.put('/transactions/:id', async (req, res) => {
  ...
  await pool.query(
    ...
    [dateStr, type, categoryId || null, accountId, transferToId || null, amount, note || null, req.params.id]
  );
  res.json({ ok: true });
});
